...
module Pandatone
  class NavigationTest < ApplicationSystemTestCase
    ...
    # The masthead is the host's: its mark, its destinations, its door. What
    # the engine hands it is its own three sections, in the order you work in
    # them, and the host puts them where its destinations go.
    SECTIONS = [ "Colors", "Palettes", "Lookup" ].freeze

    test "offers its sections to the host's nav, colors first" do
      visit root_path

      assert_equal SECTIONS, all(".nav a").map(&:text) & SECTIONS
    end
    ...
    test "marks the section you are in, not only the address you are at" do
      { palettes_path => "Palettes", root_path => "Palettes",
        ...
        lookup_path => "Lookup" }.each do |path, here|
        visit path

        current = all(".nav [aria-current='page']").map(&:text) & SECTIONS
        assert_equal [ here ], current, "#{path} should be #{here} and nothing else"
      end
    end

    # The engine's stylesheets and script arrive through the host's head.
    test "brings its own stylesheets and its own script" do
      visit root_path

      assert_selector "link[rel='stylesheet'][href*='pandatone/components']", visible: false
      assert_selector "script[type='module']", text: /import "pandatone"/, visible: false
    end
